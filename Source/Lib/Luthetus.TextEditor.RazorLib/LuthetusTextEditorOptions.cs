...
public record LuthetusTextEditorOptions
{
    ...
    /// <summary>Default value if left null is: <see cref="AutocompleteService"/></summary>
    public Func<IServiceProvider, IAutocompleteService> AutocompleteServiceFactory { get; init; } = serviceProvider => new AutocompleteService(serviceProvider.GetRequiredService<IAutocompleteIndexer>());
    /// <summary>Default value if left null is: <see cref="AutocompleteIndexer"/></summary>
    public Func<IServiceProvider, IAutocompleteIndexer> AutocompleteIndexerFactory { get; init; } = serviceProvider => new AutocompleteIndexer(serviceProvider.GetRequiredService<ITextEditorService>());
    ...
    /// <summary>Provide null if one wishes to not have Luthetus.CommonServices initialized from within Luthetus.TextEditor but instead, one wishes to initialize Luthetus.CommonServices themselves manually.</summary>
